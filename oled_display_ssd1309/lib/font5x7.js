...
// ─── Russian Cyrillic (basic alphabet) ───────────────────────────────────
// Hand-designed 5x7 glyphs contributed by a user. Codepoints U+0410–U+044F
// plus Ё/ё (U+0401 / U+0451). Two adjustments from the contributed set:
//   - Ё (0x0401) maps to the same glyph as Е: uppercase Е is full-height
//     with its top bar occupying the entire top row, leaving no empty row
//     for the diaeresis. Lowercase ё keeps its dots (the x-height body
//     leaves the top rows free, same as how lowercase й carries its breve).
//   - й (0x0439) carries its breve on the top pixel row (the x-height body
//     leaves that row free), so it renders as и with a breve above it.

// Uppercase А–Я
EXTENDED[0x0410] = [0x7E, 0x11, 0x11, 0x11, 0x7E];  // А
EXTENDED[0x0411] = [0x7F, 0x49, 0x49, 0x49, 0x30];  // Б
EXTENDED[0x0412] = [0x7F, 0x49, 0x49, 0x49, 0x36];  // В
EXTENDED[0x0413] = [0x7F, 0x01, 0x01, 0x01, 0x01];  // Г
EXTENDED[0x0414] = [0x60, 0x3E, 0x21, 0x21, 0x7F];  // Д
EXTENDED[0x0415] = [0x7F, 0x49, 0x49, 0x49, 0x41];  // Е
EXTENDED[0x0401] = [0x7F, 0x49, 0x49, 0x49, 0x41];  // Ё → Е (no room for diaeresis at 5x7)
EXTENDED[0x0416] = [0x77, 0x08, 0x7F, 0x08, 0x77];  // Ж
EXTENDED[0x0417] = [0x41, 0x49, 0x49, 0x49, 0x36];  // З
EXTENDED[0x0418] = [0x7F, 0x10, 0x08, 0x04, 0x7F];  // И
EXTENDED[0x0419] = [0x7F, 0x10, 0x09, 0x04, 0x7F];  // Й
EXTENDED[0x041A] = [0x7F, 0x08, 0x14, 0x22, 0x41];  // К
EXTENDED[0x041B] = [0x40, 0x3F, 0x01, 0x01, 0x7F];  // Л
EXTENDED[0x041C] = [0x7F, 0x02, 0x0C, 0x02, 0x7F];  // М
EXTENDED[0x041D] = [0x7F, 0x08, 0x08, 0x08, 0x7F];  // Н
EXTENDED[0x041E] = [0x3E, 0x41, 0x41, 0x41, 0x3E];  // О
EXTENDED[0x041F] = [0x7F, 0x01, 0x01, 0x01, 0x7F];  // П
EXTENDED[0x0420] = [0x7F, 0x09, 0x09, 0x09, 0x06];  // Р
EXTENDED[0x0421] = [0x3E, 0x41, 0x41, 0x41, 0x22];  // С
EXTENDED[0x0422] = [0x01, 0x01, 0x7F, 0x01, 0x01];  // Т
EXTENDED[0x0423] = [0x47, 0x28, 0x10, 0x08, 0x07];  // У
EXTENDED[0x0424] = [0x1C, 0x22, 0x7F, 0x22, 0x1C];  // Ф
EXTENDED[0x0425] = [0x63, 0x14, 0x08, 0x14, 0x63];  // Х
EXTENDED[0x0426] = [0x7F, 0x40, 0x40, 0x7F, 0x40];  // Ц
EXTENDED[0x0427] = [0x07, 0x08, 0x08, 0x08, 0x7F];  // Ч
EXTENDED[0x0428] = [0x7F, 0x40, 0x7F, 0x40, 0x7F];  // Ш
EXTENDED[0x0429] = [0x3F, 0x20, 0x3F, 0x20, 0x7F];  // Щ
EXTENDED[0x042A] = [0x01, 0x7F, 0x48, 0x48, 0x30];  // Ъ
EXTENDED[0x042B] = [0x7F, 0x48, 0x30, 0x00, 0x7F];  // Ы
EXTENDED[0x042C] = [0x00, 0x7F, 0x48, 0x48, 0x30];  // Ь
EXTENDED[0x042D] = [0x22, 0x41, 0x49, 0x49, 0x3E];  // Э
EXTENDED[0x042E] = [0x7F, 0x08, 0x3E, 0x41, 0x3E];  // Ю
EXTENDED[0x042F] = [0x46, 0x29, 0x19, 0x09, 0x7F];  // Я

// Lowercase а–я
EXTENDED[0x0430] = [0x20, 0x54, 0x54, 0x54, 0x78];  // а
EXTENDED[0x0431] = [0x3C, 0x4A, 0x4A, 0x49, 0x31];  // б
EXTENDED[0x0432] = [0x7C, 0x54, 0x54, 0x54, 0x28];  // в
EXTENDED[0x0433] = [0x7C, 0x04, 0x04, 0x04, 0x04];  // г
EXTENDED[0x0434] = [0x60, 0x38, 0x24, 0x24, 0x7C];  // д
EXTENDED[0x0435] = [0x38, 0x54, 0x54, 0x54, 0x18];  // е
EXTENDED[0x0451] = [0x38, 0x55, 0x54, 0x55, 0x18];  // ё
EXTENDED[0x0436] = [0x6C, 0x10, 0x7C, 0x10, 0x6C];  // ж
EXTENDED[0x0437] = [0x44, 0x44, 0x54, 0x54, 0x28];  // з
EXTENDED[0x0438] = [0x7C, 0x20, 0x10, 0x08, 0x7C];  // и
EXTENDED[0x0439] = [0x7C, 0x21, 0x11, 0x09, 0x7C];  // й (и with breve on top row)
EXTENDED[0x043A] = [0x7C, 0x10, 0x10, 0x28, 0x44];  // к
EXTENDED[0x043B] = [0x40, 0x3C, 0x04, 0x04, 0x7C];  // л
EXTENDED[0x043C] = [0x7C, 0x08, 0x10, 0x08, 0x7C];  // м
EXTENDED[0x043D] = [0x7C, 0x10, 0x10, 0x10, 0x7C];  // н
EXTENDED[0x043E] = [0x38, 0x44, 0x44, 0x44, 0x38];  // о
EXTENDED[0x043F] = [0x7C, 0x04, 0x04, 0x04, 0x7C];  // п
EXTENDED[0x0440] = [0x7C, 0x14, 0x14, 0x14, 0x08];  // р
EXTENDED[0x0441] = [0x38, 0x44, 0x44, 0x44, 0x28];  // с
EXTENDED[0x0442] = [0x04, 0x04, 0x7C, 0x04, 0x04];  // т
EXTENDED[0x0443] = [0x0C, 0x50, 0x50, 0x50, 0x3C];  // у
EXTENDED[0x0444] = [0x18, 0x24, 0x7E, 0x24, 0x18];  // ф
EXTENDED[0x0445] = [0x44, 0x28, 0x10, 0x28, 0x44];  // х
EXTENDED[0x0446] = [0x7C, 0x40, 0x40, 0x7C, 0x40];  // ц
EXTENDED[0x0447] = [0x0C, 0x10, 0x10, 0x10, 0x7C];  // ч
EXTENDED[0x0448] = [0x7C, 0x40, 0x7C, 0x40, 0x7C];  // ш
EXTENDED[0x0449] = [0x3C, 0x20, 0x3C, 0x20, 0x7C];  // щ
EXTENDED[0x044A] = [0x04, 0x7C, 0x50, 0x50, 0x20];  // ъ
EXTENDED[0x044B] = [0x7C, 0x50, 0x50, 0x20, 0x7C];  // ы
EXTENDED[0x044C] = [0x7C, 0x50, 0x50, 0x20, 0x00];  // ь
EXTENDED[0x044D] = [0x28, 0x44, 0x54, 0x54, 0x38];  // э
EXTENDED[0x044E] = [0x7C, 0x10, 0x38, 0x44, 0x38];  // ю
EXTENDED[0x044F] = [0x08, 0x54, 0x34, 0x14, 0x7C];  // я
