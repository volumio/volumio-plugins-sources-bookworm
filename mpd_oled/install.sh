#!/bin/bash
path=/data/plugins/system_hardware/mpd_oled
...
sudo apt update
sudo apt-get --assume-yes install --no-install-recommends i2c-tools cava 
#libasound2 libc6 libfftw3-double3 libiniparser1 libncursesw6 libpulse

##############################
# install CAVA if not present
ln -s /usr/bin/cava /usr/local/bin/mpd_oled_cava
#if [ ! -f "/usr/local/bin/mpd_oled_cava" ]
#then
#  echo "Installing CAVA"
#  sudo cp -p $path/cava /usr/local/bin/mpd_oled_cava
#  sudo chmod +x /usr/local/bin/mpd_oled_cava
#  sudo mkdir /usr/local/share/consolefonts
#  sudo cp $path/cava.psf /usr/local/share/consolefonts
#else
...
##############################
# install mpd_oled if not present
if [ ! -f "/usr/local/bin/mpd_oled" ]
then
  echo "Installing mpd_oled"
  sudo cp -p $path/mpd_oled /usr/local/bin/mpd_oled
