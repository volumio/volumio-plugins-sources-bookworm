...
class ControllerJellyfin implements NowPlayingPluginSupport {
  ...
  getUIConfig() {
    ...
    libQ.all(configPrepTasks).then((configParams: [any, string]) => {
      ...
      // Add Server section
      addServerUIConf.content[3].value = jellyfin.getConfigValue('testServerConnBeforeAdd');
  ...
  async configAddServer(data: any) {
    ...
    if (testBeforeAdd && this.#serverPoller) {
      jellyfin.toast('info', jellyfin.getI18n('JELLYFIN_TESTING_CONN'));
      const conn = await this.#serverPoller?.testConnection(url);
      if (conn.error) {
        jellyfin.toast('error', jellyfin.getI18n('JELLYFIN_TEST_CONN_FAILED', conn.error));
        return;
      }
      jellyfin.toast('success', jellyfin.getI18n('JELLYFIN_TEST_CONN_OK'));
    }
    ...
    jellyfin.setConfigValue('testServerConnBeforeAdd', testBeforeAdd);
