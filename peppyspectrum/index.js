...
peppyspectrum.prototype.delayspectrum = function (data) {
    const self = this;

    const defer = libQ.defer();
    var delayspectrum = data['delayspectrum'];
    self.config.set('delayspectrum', delayspectrum);
    try {

        fs.readFile(__dirname + "/startpeppyspectrum.sh.tmpl", 'utf8', function (err, data) {
            if (err) {
                defer.reject(new Error(err));
                return console.log(err);
            }

            const conf1 = data.replace("${delayspectrum}",delayspectrum )

            fs.writeFile("/data/plugins/user_interface/peppyspectrum/startpeppyspectrum.sh", conf1, 'utf8', function (err) {
                if (err)
                    defer.reject(new Error(err));
                else defer.resolve();
            });
