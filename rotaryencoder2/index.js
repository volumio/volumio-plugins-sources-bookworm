...
rotaryencoder2.prototype.onStart = function() {
	...
	self.installOverlays([...Array(maxRotaries).keys()])
	.then(_ => {return self.attachListeners([...Array(maxRotaries).keys()])})
...
rotaryencoder2.prototype.onStop = function() {
	...
	self.uninstallOverlays([...Array(maxRotaries).keys()])
...
rotaryencoder2.prototype.installOverlays = function (rotaryIndexArray) {
	var self = this;
	var defer = libQ.defer();
	var rotaryIndex;

	if (self.debugLogging) self.logger.info('[ROTARYENCODER2] installOverlays: ' + rotaryIndexArray.map(i =>  {return i + 1}));

	if (Array.isArray(rotaryIndexArray)){
		if (rotaryIndexArray.length > 0) {
			rotaryIndex = rotaryIndexArray[rotaryIndexArray.length - 1];
			return self.installOverlays(rotaryIndexArray.slice(0,rotaryIndexArray.length - 1))
			.then(_ => {
			...
			if (self.debugLogging) self.logger.info('[ROTARYENCODER2] installOverlays: end of recursion.');
			defer.resolve();
		}
	} else {
		self.logger.error('[ROTARYENCODER2] installOverlays: rotaryIndexArray must be an Array');
...
rotaryencoder2.prototype.uninstallOverlays = function (rotaryIndexArray) {
	var self = this;
	var defer = libQ.defer();
	var rotaryIndex;
	var overlays = [];

	if (self.debugLogging) self.logger.info('[ROTARYENCODER2] uninstallOverlays: ' + rotaryIndexArray.map(i =>  {return i + 1}));

	if (Array.isArray(rotaryIndexArray)){
		if (rotaryIndexArray.length > 0) {
			rotaryIndex = rotaryIndexArray[0];
			self.uninstallOverlays(rotaryIndexArray.slice(1,rotaryIndexArray.length))
			.then(_ => {
				if (self.config.get('enabled'+rotaryIndex)) {
					...
					.then(_ => {
						if (self.debugLogging) self.logger.info('[ROTARYENCODER2] uninstallOverlays: overlays to remove: ' + overlays);
						return defer.resolve(overlays);
					})
					.fail(msg => {
						if (self.debugLogging) self.logger.error('[ROTARYENCODER2] uninstallOverlays: failed to lookup overlays for ' + (rotaryIndex + 1));
			...
			if (self.debugLogging) self.logger.info('[ROTARYENCODER2] uninstallOverlays: end of recursion.');
			defer.resolve();
		}
	} else {
		self.logger.error('[ROTARYENCODER2] uninstallOverlays: rotaryIndexArray must be an Array: ' + rotaryIndexArray);
		defer.reject('rotaryIndexArray must be an Array of integers')
	}
	return defer.promise;
}
...
rotaryencoder2.prototype.attachListeners = function (rotaryIndexArray) {
	var self = this;
	var defer = libQ.defer();
	var rotaryIndex;

	if (self.debugLogging) self.logger.info('[ROTARYENCODER2] attachListeners: ' + rotaryIndexArray.map(i =>  {return i + 1}));

	if (Array.isArray(rotaryIndexArray)){
		if (rotaryIndexArray.length > 0) {
			rotaryIndex = rotaryIndexArray[rotaryIndexArray.length - 1];
			return self.attachListeners(rotaryIndexArray.slice(0,rotaryIndexArray.length - 1))
			.then(_ => {
				if (self.config.get('enabled'+rotaryIndex)) {
					return self.checkOverlayExists(rotaryIndex, 'rotary')
					.then(idx => {
						if (idx > -1) {
							if (self.debugLogging) self.logger.info('[ROTARYENCODER2] attachListeners: attach rotary ' + (rotaryIndex + 1));
					...
					.then(idx => {
						if (idx > -1) {
							if (self.debugLogging) self.logger.info('[ROTARYENCODER2] attachListeners: attach button '  + (rotaryIndex + 1));
					...
					.fail(err => {
						if (self.debugLogging) self.logger.error('[ROTARYENCODER2] attachListeners: failed: ' + err);
					...
					if (self.debugLogging) self.logger.info('[ROTARYENCODER2] attachListeners rotaries: ' + JSON.stringify(self.handles));
					if (self.debugLogging) self.logger.info('[ROTARYENCODER2] attachListeners buttons: ' + JSON.stringify(self.buttons));
					return defer.resolve();
				}
			})
		} else {
			if (self.debugLogging) self.logger.info('[ROTARYENCODER2] attachListeners: end of recursion.');
			defer.resolve();
		}
	} else {
		self.logger.error('[ROTARYENCODER2] attachListeners: rotaryIndexArray must be an Array');
		defer.reject('rotaryIndexArray must be an Array of integers')
	}
	return defer.promise;
}
...
rotaryencoder2.prototype.activateButtons = function (rotaryIndexArray) {
	...
	if (Array.isArray(rotaryIndexArray)){
		if (rotaryIndexArray.length > 0) {
			rotaryIndex = rotaryIndexArray[rotaryIndexArray.length - 1];
			return self.activateButtons(rotaryIndexArray.slice(0,rotaryIndexArray.length - 1))
			.then(_=> {
				if (self.config.get('enabled'+rotaryIndex)) {
					...
					if (Number.isInteger(gpio) && (gpio > 0)) {
						...
						if (self.debugLogging) self.logger.info('[ROTARYENCODER2] activateButtons: Now assign push button for Rotary: ' + (rotaryIndex + 1) + ' on GPIO: '+ gpio + ', debounce: ' + debounce + ', activeLow: ' + self.config.get('pushState'+rotaryIndex) + ',gpioPull: ');
						return self.addOverlay(gpio, undefined, undefined, self.config.get('pushState'+rotaryIndex),'up')
						.then(_=>{
							return self.attachListener(gpio, 'button');
						})
						.then(handle => {
							self.addEventHandle(handle, rotaryIndex,'button')
						})
						.fail(err => {
							self.commandRouter.pushToastMessage('error', self.getI18nString('ROTARYENCODER2.TOAST_WRONG_PARAMETER'), self.getI18nString('TOAST_ERR_ACT_ROTARY_FAILED'));
							return defer.reject()
	...
	if (Array.isArray(rotaryIndexArray)){
		if (rotaryIndexArray.length > 0) {
			...
			.then(_=>{
				...
				if (self.config.get('enabled'+rotaryIndex)) {
					if (Number.isInteger(btnGPIO) && (btnGPIO > 0)) {				
						if (self.debugLogging) self.logger.info('[ROTARYENCODER2] deactivateButtons: Calling detachListener for button: ' + btnGPIO + ' of Rotary: ' + (rotaryIndex+1));
						return self.detachListener(self.buttons[rotaryIndex])
						// .then(_=>{ return self.checkOverlayExists(rotaryIndex, type='button')})
						// .then(idx=>{if (idx > -1) return self.removeOverlay(idx)})
						.then(_=>{
							if (self.debugLogging) self.logger.info('[ROTARYENCODER2] deactivateButtons: deactivated button for rotary' + (rotaryIndex + 1));
							return defer.resolve();
						})
						.fail(msg => {
							if (self.debugLogging) self.logger.error('[ROTARYENCODER2] deactivateButtons: failed to deactivate button for rotary' + (rotaryIndex + 1));
							self.commandRouter.pushToastMessage('error', self.getI18nString('ROTARYENCODER2.TOAST_WRONG_PARAMETER'), self.getI18nString('ROTARYENCODER2.TOAST_KILL_HANDLE_FAIL'));
							return defer.resolve();
						})
					} else {
						if (self.debugLogging) self.logger.info('[ROTARYENCODER2] deactivateButtons: No button configured for Rotary: ' + (rotaryIndex + 1));
						return defer.resolve()
					}
				} else {
					if (self.debugLogging) self.logger.info('[ROTARYENCODER2] deactivateButtons: Rotary: ' + (rotaryIndex + 1) + 'is disabled.');
					return defer.resolve()
				}
			})
		} else {
...
rotaryencoder2.prototype.addEventHandle = function (handle, rotaryIndex, handleType = "rotary") {
	...
	if (handleType == "rotary") {
		...
		self.buttons[rotaryIndex]=handle;
		self.buttons[rotaryIndex].stdout.on("data", function (chunk) {
			var i=0;
			while (chunk.length - i >= 16) {
				var type = chunk.readUInt16LE(i+8)
				//var code = chunk.readUInt16LE(i+10) //would additionally read the key-code assigned to a button, but we do not need this for the plugin
		...
		self.buttons[rotaryIndex].stdout.on('end', function(){
			if (self.debugLogging) self.logger.info('[ROTARYENCODER2] addEventHandle: Stream from button ended.');
		});
		self.buttons[rotaryIndex].stderr.on('data', (data) => {
			self.logger.error('[ROTARYENCODER2] addEventHandle: ' + `stderr: ${data}`);
			self.commandRouter.pushToastMessage('error', self.getI18nString('ROTARYENCODER2.TOAST_WRONG_PARAMETER'), self.getI18nString('ROTARYENCODER2.TOAST_ERR_FROM_STREAM') + '(' + data + ')');
		});
		self.buttons[rotaryIndex].on('close', (code) => {
			if (self.debugLogging) self.logger.info('[ROTARYENCODER2] addEventHandle: ' + `child process exited with code ${code} `);
		});		
	}
}
