...
# Check if the separate RoonBridge plugin is installed — if so, don't remove shared files
ROONBRIDGE_PLUGIN_DIR="/data/plugins/music_service/RoonBridge"
if [ -d "$ROONBRIDGE_PLUGIN_DIR" ]; then
    echo "RoonBridge plugin detected at $ROONBRIDGE_PLUGIN_DIR"
    echo "Keeping RoonBridge installation and service for the other plugin."
    echo "Only removing MetaRoon-specific configuration."
else
