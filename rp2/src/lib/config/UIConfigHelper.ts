import {
  type ObservedUIConfig,
  type ObservedUIConfigSection,
  type ObservedUIConfigSectionContent,
  type UIConfig,
  type UIConfigSection
} from './ui-config-types';
import { type UIConfigSectionKey } from './ui-config-schema';
...
export default class UIConfigHelper {
  ...
  static #observeSection<K extends UIConfigSectionKey>(
    data: UIConfigSection<K>
  ): ObservedUIConfigSection<K> {
    if (!data.content) {
      data.content = [];
    }
  ...
  static #observeSectionContent<K extends UIConfigSectionKey>(
    data: NonNullable<UIConfigSection<K>['content']>
  ): ObservedUIConfigSectionContent<K> {
    return new Proxy(data, {
      get: (target, prop) => {
