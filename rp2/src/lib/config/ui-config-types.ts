import {
  type UIConfigElementOf,
  type UIConfigSectionContentKeyOf,
  type UIConfigSectionKey
...
export interface UIConfigInput<
  ...
  T extends UIConfigInputType
> extends UIConfigElementBase<K> {
  element: 'input';
  type: T;
...
export interface UIConfigSelect<
  K extends UIConfigSectionKey
> extends UIConfigElementBase<K> {
  element: 'select';
  doc?: string;
...
export interface UIConfigButton<
  K extends UIConfigSectionKey
> extends UIConfigElementBase<K> {
  element: 'button';
  onClick:
...
export interface UIConfigSwitch<
  K extends UIConfigSectionKey
> extends UIConfigElementBase<K> {
  element: 'switch';
  value: boolean;
...
export type ObservedUIConfigSection<K extends UIConfigSectionKey> =
  UIConfigSection<K> & {
    content: ObservedUIConfigSectionContent<K>;
  };

export type ObservedUIConfigSectionContent<K extends UIConfigSectionKey> =
  UIConfigSection<K>['content'] & {
