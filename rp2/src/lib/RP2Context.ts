...
class RP2Context {
  ...
  refreshUIConfig() {
    this.#pluginContext.coreCommand
  ...
  getRpjsLib() {
    ...
    if (!rpjs) {
      const sessionData =
        this.getConfigValue('persistSession') ?
  ...
  #getPlayer() {
    let player = this.get<MPVPlayer>(STORE_KEYS['player']);
    if (!player) {
  ...
  getStateTransformer() {
    let transformer = this.get<StateTransformer>(
      STORE_KEYS['stateTransformer']
    );
    if (!transformer) {
  ...
  async reset() {
    const rpjs = this.get<RadioParadise>(STORE_KEYS['rp.js']);
    if (rpjs) {
      await rpjs.dispose();
      rpjs.removeAllListeners();
      this.delete(STORE_KEYS['rp.js']);
    }
    const player = this.get<MPVPlayer>(STORE_KEYS['player']);
    if (player) {
      await player.quit();
